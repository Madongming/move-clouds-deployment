...
func N2ISingleDeployment(ctx *framework.TestContext, f *framework.Framework) {
	var (
	...
	Context("Test singledeployment update from mod nodeport to ingress", func() {
		It("Should be create singledeployment mode nodeport succuess", func() {
			gvr := schema.GroupVersionResource{
				Group:    "deployment.github.com",
				Version:  "v1",
				Resource: "singledeployments",
			}
			objNodeport, err = dc.Resource(gvr).Namespace("default").Create(context.TODO(), objNodeport, metav1.CreateOptions{})
			Expect(err).Should(BeNil())
		})

		It("Should be exsit singledeployment mode nodeport", func() {
		...
		It("Should be update singledeployment to mode ingress succuess", func() {
			gvr := schema.GroupVersionResource{
				Group:    "deployment.github.com",
				Version:  "v1",
				Resource: "singledeployments",
			...
			// Get objnodeport update need
			objNodeport, err = dc.Resource(gvr).Namespace("default").Get(context.TODO(), objNodeport.GetName(), metav1.GetOptions{})
			Expect(err).Should(BeNil())
			objIngress.SetResourceVersion(objNodeport.GetResourceVersion())

			_, err := dc.Resource(gvr).Namespace("default").Update(context.TODO(), objIngress, metav1.UpdateOptions{})
			Expect(err).Should(BeNil())
		})
		...
		It("Should be delete success", func() {
			...
			err := dc.Resource(gvr).Namespace("default").Delete(context.TODO(), objIngress.GetName(), metav1.DeleteOptions{})
			Expect(err).Should(BeNil())
		})
